...
class OneHotEncodeFromTags(BaseEstimator, TransformerMixin):
    ...
    def __init__(
        self, cols: Union[str, List[str]], order_by: Optional[Union[Literal["popularity", "alphabet"], None]] = None
    ):
    ...
    def get_unique_tags(self, x: Union[np.ndarray, pd.Series]) -> List:
    ...
    def transform(self, X, y=None) -> pd.DataFrame:
        ...
        new_tags: Dict[str, Union[List[str], None]] = {col: None for col in self.cols}
...
class ColumnDropper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X, y=None) -> pd.DataFrame:
        cols = [col for col in self.cols if col in X.columns]
